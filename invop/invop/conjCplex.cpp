...
int tipoAlgoritmo;
int limiteCortesClique;
int limiteCortesCiclo;
int limiteCortesIter;
...
int main(int argc, char **argv)
{
	...
	if(argc != 5){
		cout << "Se pasaron mal los paramtros" << endl;
		cout << "Parametro 1: el tipo de Algoritmo. 1 = branch and bound, 2 = branch and cut, 3 = cut and branch" << endl;
		cout << "Parametro 2: la cantidad de Cortes Cliques maxima que seran evaludas para ver si se agregan cortes" << endl;
		cout << "Parametro 3: la cantidad de Cortes Ciclos Impares maxima que seran evaludas para ver si se agregan cortes" << endl;
		cout << "Parametro 4 : la cantidad maxima de cortes que se agregan por cada iteracion "<< endl;
	}

	tipoAlgoritmo =  atoi(argv[1]);
	cout << "el algoritmo " << tipoAlgoritmo << endl;
	int limiteCortesClique = atoi(argv[2]);
	cout << "limiteCortesClique " << limiteCortesClique << endl;
	
	limiteCortesCiclo = atoi(argv[3]);
	cout << "limiteCortesCiclo " << limiteCortesCiclo << endl;
	
	limiteCortesIter = atoi(argv[4]);
	cout << "limiteCortesIter " << limiteCortesIter << endl;
	...
	if(tipoAlgoritmo == 1)	// si es branch and bound no voy a cortar, luego pongo restricciones sin nada
	{
		vector <int> elvec;
		vectorRestriccionesCiclos.push_back(elvec);
		vectorRestriccionesCliques.push_back(elvec);

	}else{		// si no es branch and bound busco ciclos y cliques
		g.cliques(vectorRestriccionesCliques, limiteCortesClique);
		g.ciclosImpares(vectorRestriccionesCiclos, limiteCortesCiclo);
	}
	...
	if (limiteCortesIter > cantCortes)
	{
		cout << "los limites por iteracion es menor a la cantidad de cortes, cambio" << endl;
		limiteCortesIter = cantCortes;
	}
	...
	int        *useraction_p)
{
    ...
    *useraction_p = CPX_CALLBACK_DEFAULT; 
    ...
    if ( (tipoAlgoritmo == 3) && (numiinf != 0) )	// estoy en la raiz entonces aplico los cortes
    {
    	//cout << "no estoy en el nodo 0, no corto" << endl;
    	return 0;
    }
    ...
    sort(cutvios.begin(), cutvios.end(), funcionComparar);
		...
		for ( int i = 0 ; i < limiteCortesIter; i++) {
			int elcorte = cutvios[i].second;
			if(cutvios[i].first > 0.01){ 
				 cout << "el nodo en que voy a cortar es " << numiinf ;
...
//	goto TERMINATE;
    ...
    for (i = 0; i < cuts; i++) {
		...
		cout << "el rhs es " << (vectorRestriccionesCiclos[i-vectorRestricciones.size()].size()) << endl;
		}
	}
