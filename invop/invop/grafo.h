...
class grafo{
    public:
        ...
        void devolverCicloImpar(int nodoInicial, int nodo, int nodoFinal,const vector<int> &predecesor, vector < vector <int> > &elvec, vector<bool> &marcado){
            ...
            if (contador % 2 == 1 && contador > 1){     // me fijo si es impar
                ...
                if (i != vec.size() -1){
                    ...
                    {
                        cout << "TA TODO MAL no hay arista entre " << vec[i] + 1 << " y " << vec[i+1] + 1  << endl;
                    }
                }else{
                    if( !hayArista(vec[0], vec[vec.size()-1]) )
                    {
                        cout << "TA TODO MAL ultimo no hay arista entre " << vec[0]+1 << " y " << vec[vec.size()-1] +1  << endl;
                    }
                }
                }
        ...
        int dfsRecur(int nodoInicial, int nodo, vector <int> &predecesor, int contador, vector< vector <int> > & vec2vec, vector<bool> &marcado){
            ...
            //cout << "me llaman con el nodo " << nodo << endl;
            for (int i = 0; i < vecinos(nodo).size(); i++)
            {
                ...
                if(predecesor[elvecino] == -1){
                    ...
                    //&& elvecino != predecesor[nodo]){		// encontre un ciclo impar
	...
	/*		for (int k = 0; k < cantCortes; k++)
            ...
            cout << "hago " << temp.size() << " ciclosImpares, el mas grande de " << temp[0].size() << endl;
